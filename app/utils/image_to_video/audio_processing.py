...
async def process_narrator_audio(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Only use narrator_speech_text if narrator_audio_url is not provided
            ...
            # Generate audio data using unified TTS service
            logger.info(f"Generating speech with voice: {voice}")
            audio_data, provider_used = await tts_service.generate_speech(
                text=speech_text,
                voice=voice,
            ...
            logger.info(f"Generated speech using {provider_used} provider")
