...
async def process_image_to_video(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"zoom_speed: {zoom_speed}")
        ...
        # Step 5: Generate video from image
        logger.info("Step 5: Generating video from image with effects")
        srt_path = None
        srt_url = None
        ...
        # Step 8: Add captions using advanced caption service if requested
        if params.get("should_add_captions", False) and speech_text:
            logger.info("Step 8: Adding captions using advanced caption service")
            try:
                # Upload the intermediate video to get a URL for caption processing
                from app.utils.image_to_video.output_processing import upload_to_storage
                video_url, _ = await upload_to_storage(final_video_path)
                
                # Use advanced caption service to add captions
                from app.services.video.caption_service import advanced_caption_service
                import uuid
                
                caption_params = {
                    'video_url': video_url,
                    'captions': speech_text,
                    'settings': params.get("caption_properties", {}),
                    'language': 'auto'
                }
                
                caption_result = await advanced_caption_service.process_caption_job(
                    f"image_to_video_captions_{uuid.uuid4().hex[:8]}", 
                    caption_params
                )
                
                # Return result directly from caption service since it already uploaded the file
                logger.info(f"Added captions to video: {caption_result['url']}")
                return {
                    "final_video_url": caption_result['url'],
                    "final_video_path": caption_result['path'],
                    "has_audio": audio_path is not None,
                    "has_captions": True,
                    "video_duration": None,  # Could be calculated from the original video
                    "srt_url": caption_result.get('srt_url'),
                    "metadata": {
                        "width": caption_result.get('width'),
                        "height": caption_result.get('height'),
                        "file_size": caption_result.get('file_size'),
                        "style": caption_result.get('style', 'classic')
                    }
                }
        ...
        logger.info("Step 9: Preparing result")
        result = await prepare_result(
            video_path=final_video_path,
            has_audio=audio_path is not None,
            has_captions=params.get("should_add_captions", False) and speech_text is not None,
            srt_url=srt_url
        )
