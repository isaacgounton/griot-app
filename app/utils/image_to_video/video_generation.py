...
async def create_video_with_effects(
    ...
    try:
        ...
        logger.info(f"Video parameters: zoom_speed={zoom_speed}, video_length={video_length}, frame_rate={frame_rate}")
        logger.info(f"Output dimensions: {output_dims}, Scale dimensions: {scale_dims}")
        
        # Validate effect_type parameter
        valid_effects = ['none', 'zoom', 'zoom_out', 'pan', 'ken_burns']
        if effect_type not in valid_effects:
            logger.error(f"INVALID EFFECT TYPE DETECTED: '{effect_type}' - Valid types are: {valid_effects}")
            effect_type = 'zoom'  # Fallback to zoom
            logger.info(f"Falling back to zoom effect")
        else:
            logger.info(f"Effect type '{effect_type}' is valid, proceeding with generation")
        ...
        if effect_type == "none":
            logger.info("EXECUTING: No motion effect - static image conversion")
            ...
            filter_complex = f"scale=w={width}:h={height}:force_original_aspect_ratio=increase,crop={width}:{height}"
            ...
            logger.info(f"EXECUTING: Smooth zoom effect with speed {zoom_speed}")
            # Improved smooth zoom effect with easing
            # Use a smoother zoom progression with cubic easing
            zoom_progress = f"(on/{total_frames})"
            # Apply cubic easing for smoother motion: t^3 for ease-in-out
            eased_progress = f"({zoom_progress}*{zoom_progress}*{zoom_progress})"

            filter_complex = (
                f"scale={scale_dims},"
                f"zoompan=z='1+({zoom_speed_normalized}*{video_length})*{eased_progress}':"
            ...
            logger.info(f"EXECUTING: Smooth zoom-out (reveal) effect with speed {zoom_speed}")
            # Zoom-out: start zoomed in, pull back to reveal the full image
            zoom_progress = f"(on/{total_frames})"
            eased_progress = f"({zoom_progress}*{zoom_progress}*{zoom_progress})"

            filter_complex = (
                f"scale={scale_dims},"
                f"zoompan=z='max(1,{zoom_factor}-({zoom_speed_normalized}*{video_length})*{eased_progress})':"
                f"d={total_frames}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
                f"s={output_dims},fps={frame_rate}"
            )
            ...
            logger.info(f"EXECUTING: Pan effect with direction '{pan_direction}'")
            ...
            logger.info("EXECUTING: Ken Burns effect")
            ...
            logger.warning(f"UNKNOWN EFFECT TYPE: {effect_type}. Using static image conversion.")
            # Use a simpler filter chain to avoid potential syntax issues
            filter_complex = f"scale=w={width}:h={height}:force_original_aspect_ratio=increase,crop={width}:{height}"
...
async def combine_video_audio_captions(
    ...
    try:
        ...
        if srt_path:
            ...
            if is_ass:
                # For ASS subtitles, use the subtitles filter directly with file path
                # Don't add as input since we're referencing the file directly
...
def prepare_subtitle_styling(caption_properties: Optional[Dict] = None) -> Dict[str, str]:
    ...
    # Update with user preferences if provided (field names match VideoCaptionProperties model)
    if "font_family" in caption_properties:
        style_options["Fontname"] = caption_properties["font_family"]

    if "font_size" in caption_properties:
        style_options["Fontsize"] = str(caption_properties["font_size"])

    if "line_color" in caption_properties:
        style_options["PrimaryColour"] = caption_properties["line_color"]

    if "background_color" in caption_properties:
        style_options["BackColour"] = caption_properties["background_color"]

    if "outline_width" in caption_properties:
        style_options["Outline"] = str(caption_properties["outline_width"])

    if "spacing" in caption_properties:
        style_options["Spacing"] = str(caption_properties["spacing"])
    ...
    if "alignment" in caption_properties:
        alignment = caption_properties["alignment"]
        # Map text alignment to ASS alignment codes
        base = int(style_options.get("Alignment", "2"))
        row = (base - 1) // 3 * 3  # 0=bottom, 3=middle, 6=top
        if alignment == "left":
            style_options["Alignment"] = str(row + 1)
        elif alignment == "center":
            style_options["Alignment"] = str(row + 2)
        elif alignment == "right":
            style_options["Alignment"] = str(row + 3)
