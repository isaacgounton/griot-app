...
def build_ffmpeg_filter_complex(
    ...
    for i, overlay in enumerate(sorted_overlays):
        ...
        # Add colorkey filter if enabled
        colorkey_filter = ""
        if overlay.get('colorkey_enabled', False):
            colorkey_color = overlay.get('colorkey_color', 'green')
            colorkey_similarity = overlay.get('colorkey_similarity', 0.1)
            colorkey_blend = overlay.get('colorkey_blend', 0.1)
            colorkey_filter = f",colorkey=color={colorkey_color}:similarity={colorkey_similarity}:blend={colorkey_blend}"
        ...
        overlay_filter = f"[{input_index}:v]{scale_filter}{colorkey_filter}{opacity_filter}[overlay{i}]"
