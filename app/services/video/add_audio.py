...
class AddAudioService:
    ...
    async def process_job(self, job_id: str, job_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            output_filename = f"mixed_video_{job_id}_{unique_suffix}{video_ext}"
    ...
    def _build_ffmpeg_standard_command(self, video_path: str, audio_path: str, 
        ...
        if has_audio_stream:
            # Video has audio, mix both audio streams with proper normalization
            # Use amix with normalize=0 to prevent automatic gain adjustment and weights for better control
            filter_complex = f"[0:a]volume={video_vol}[a1];[1:a]volume={audio_vol}[a2];[a1][a2]amix=inputs=2:duration=shortest:normalize=0:weights='1 1'[aout]"
        else:
        ...
        return [
            ...
            "-c:v", "copy",
            "-c:a", "aac",
            "-b:a", "128k",  # Set audio bitrate for consistent quality
            "-shortest",
            output_path
        ]
    ...
    def _build_ffmpeg_loop_audio_command(self, video_path: str, audio_path: str, 
        ...
        if has_audio_stream:
            # Video has audio, mix both audio streams with proper normalization
            filter_complex = f"[0:a]volume={video_vol}[a1];[1:a]volume={audio_vol}[a2];[a1][a2]amix=inputs=2:duration=first:normalize=0:weights='1 1'[aout]"
        else:
            # Video doesn't have audio, just use the audio file with adjusted volume
            filter_complex = f"[1:a]volume={audio_vol}[aout]"
