...
def get_sage(
    ...
    # Initialize knowledge base safely - if it fails, continue without it
    knowledge = None
    try:
        if db_url:
            # Wrap PgVector creation in try-except to handle connection issues
            try:
                vector_db = PgVector(
                ...
                knowledge = Knowledge(
                    ...
                    description="Knowledge base containing user-provided documents and information for the Sage agent",
                    vector_db=vector_db,
                )
            except Exception as ve:
                import logging
                logging.warning(f"Failed to initialize PgVector for Sage: {ve}. Knowledge base will be disabled.")
                knowledge = None
    except Exception as e:
        # Log error but continue - agent will work without knowledge base
        import logging
        logging.warning(f"Failed to initialize Sage knowledge base: {e}. Agent will work without KB.")
        knowledge = None
    ...
    return Agent(
        ...
        # Knowledge base for the agent (optional - will be None if initialization failed)
        knowledge=knowledge,
        # Enable agentic RAG - gives agent a search_knowledge_base tool
        ...
        markdown=True,
        # Add datetime context for relevance
        add_datetime_to_context=True,
        # Include recent conversation history in context
        add_history_to_context=True,
        num_history_runs=5,
        # Add a tool to read the full chat history if needed
