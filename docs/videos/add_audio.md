...
| audio_volume | integer | No | Volume level for the added audio track (0-100, default: 20) |
| match_length | string | No | Whether to match the output length to "video" or "audio" (default: "video") |
| sync | boolean | No | If true, return response immediately. If false (default), create async job |
| sync_mode | string | No | Audio synchronization mode: "replace", "mix", or "overlay" (default: "replace") |
| fade_in_duration | number | No | Duration in seconds for audio fade-in effect (0.0 to 10.0) |
| fade_out_duration | number | No | Duration in seconds for audio fade-out effect (0.0 to 10.0) |

### Sync Mode Options

| Option | Description |
|--------|-------------|
| sync=false | Creates an asynchronous job and returns a job ID for tracking progress (default) |
| sync=true | Processes the request immediately and returns the result directly |

### Audio Sync Modes

| Mode | Description |
|------|-------------|
| replace | Completely replace the original video audio with the new audio (default) |
| mix | Blend the original and new audio together |
| overlay | Layer the new audio over the original audio |
...
```json
{
  ...
  "status": "completed",
  "result": {
    "url": "https://s3.amazonaws.com/bucket/output_video.mp4",
    "path": "output/videos/mixed_video_abc123.mp4",
    "duration": 120.5
  }
}
```
...
```bash
curl -X POST \
  https://localhost:8000/v1/videos/add-audio \
...
```
GET /v1/videos/add-audio/{job_id}
```
...
```json
{
  ...
  "status": "completed",
  "result": {
    "url": "https://cdn.localhost:8000/output/videos/mixed_video_j-123e4567.mp4",
...
```bash
curl -X GET \
  https://localhost:8000/v1/videos/add-audio/j-123e4567-e89b-12d3-a456-426614174000 \
  -H 'X-API-Key: your-api-key'
```
